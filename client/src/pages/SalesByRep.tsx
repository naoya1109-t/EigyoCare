...
import { fetchSalesByRep, fetchRepMonths, RepSalesRow } from "../api/sales";
import { getCurrentUser } from "../api/session";

type SortOrder = "salesDesc" | "repCode";

export default function SalesByRep() {
  const [months, setMonths] = useState<string[]>([]);
  const [month, setMonth] = useState("");
  const [data, setData] = useState<RepSalesRow[]>([]);
  const [sortOrder, setSortOrder] = useState<SortOrder>("salesDesc");
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const sortedData = [...data].sort((a, b) =>
    sortOrder === "repCode" ? a.repCode - b.repCode : b.salesAmount - a.salesAmount,
  );

  useEffect(() => {
    fetchRepMonths().then((list) => {
      setMonths(list);
  ...
  useEffect(() => {
    if (!month) return;
    setLoading(true);
    fetchSalesByRep(month)
      .then(setData)
      .catch(() => setError("担当者別売上の取得に失敗しました"))
  ...
  return (
    <AppLayout userName={getCurrentUser()?.userId ?? "ログインユーザー"}>
      ...
      <p className="mb-4 text-sm text-slate-500">対象月における担当者別の売上・粗利を比較します。</p>
      <FilterForm>
        <SelectField
          label="対象年月"
          value={month}
