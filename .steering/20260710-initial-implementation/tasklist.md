...
- [ ] 全画面について、スマートフォンブラウザでの表示確認（レスポンシブ）
  > この開発環境にはブラウザ操作ツールがなく、目視確認ができていない。curl でのHTTP到達性・ビルド成功は確認済みだが、レイアウト崩れ等の視覚的な確認は別途、実ブラウザでの手動確認が必要
- [x] 全APIエンドポイントについて、未認証アクセス時に401が返ることを確認
- [x] 表示データと基幹システム（SQL Server）上のデータの一致確認
- [x] Lint・型チェック（`npm run lint` 等）の実行、エラーゼロを確認
- [x] 主要なビジネスロジック（`server/src/services/`）・共通コンポーネントに対するテスト（Vitest）の作成・実行
- [x] IISワーカープロセスのリサイクルを模したセッション永続化の確認（開発機では代替手段での確認、または検証環境での確認）
