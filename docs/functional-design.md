...
| 4 | 顧客情報（得意先） | `dbo.ET0020得意先` | 得意先CD(PK,int), 得意先名, 郵便番号, 県CD(→`ET0001県`), 住所1/2, TEL, FAX, 営業担当CD, 締日 等（全43列） | `検索対象外`列（tinyint）は値が`1`（2291件）と`2`（7333件）のみで、`1`が検索対象外を意味する。顧客情報一覧（`GET /api/customers`）は`検索対象外 = 2`のみを抽出する（指示により2026-07-13追加）。顧客詳細（`GET /api/customers/:code`）にはこの絞り込みは適用していない。`締日`列は`5,10,15,20,25,31`のみの値で、`31`が全体の約89%（8557件）を占める。`31`は「末日（月末）」を意味する慣用表現と判断し、画面では`31`→「末日締」、それ以外→「N日締」と表示する。続けて`回収サイクル`（0=当月,1=翌月,2=翌々月,3=3ケ月,4=4ケ月,5=5ケ月,6=6ケ月。実データは0〜5のみ確認）と`回収日`+「日」を1行にまとめて表示する（例:「20日締 翌月 20日」）。顧客詳細画面から`担当者部署`/`担当者役職`/`担当者名`（先方担当者名）は削除し、代わりに`価格表FAX`/`価格表WEB`列（varchar(2)、値は`N`/`オ`/`A1`等のコード）を「価格表」という1項目にまとめて`FAX:{値} / WEB:{値}`の形式で表示する（2026-07-13、フィードバックにより変更）。続けて`送料規定`（nvarchar(100)）を「送料規定」として表示し、その下に`送料無料価格`（money）を「無料卸価格」、`倉庫出荷送料`（money）を「倉庫送料」、`返品手数料`（money）を「返品手数料」として1行ずつ、末尾に「円」を付けて表示する。`得意先CD`は左パネルの縦の長さを抑えるため、パネル内の行ではなく画面タイトル「顧客詳細」の右にバッジ表示に変更した。**`得意先名`は先頭・末尾に半角/全角スペースが入っている得意先が20件以上あることが判明**（例: CD=3「 松本義肢製作所」）。一覧・詳細とも表示前に`LTRIM(RTRIM(...))`でトリムする（2026-07-13、一覧で表示が左にずれて見えるとのフィードバックにより対応） |
...
> - `/api/budget/progress?repCode=&period=` — `period`省略時は`ET0005期`の`現在='1'`の期を使用。達成率は実績データが存在する月のみを対象に計算（未経過月は含まない）。`repCode`省略時は全担当者の予算・実績を合算した「全社計」を返す（2026-07-13追加。フィードバックにより、担当者だけでなく全社の予算進捗も見たいという要望に対応）。レスポンスには当月を含む「経過月分」の`achievementRate`に加え、当月を除いた「前月まで」の`priorMonthsAchievementRate`も含む（当月はまだ実績が確定していないため）。期首（10月）に閲覧した場合はその期に前月が存在しないため`null`を返し、画面側では「集計対象月なし」と表示する（0%と区別するため）。`totalBudgetPrior`/`totalActualPrior`（前月までの予算計・実績計）も含み、画面ではゲージにカーソルを合わせると内訳を表示する。また`ProgressGauge`は達成率が100%を超える場合（103%等）、リングの塗りは1周で頭打ちにしつつ、中央の数値表示は実際の値をそのまま出す（以前は100%に切り詰めて表示していたが、実態と合わないためフィードバックにより修正）
