...
| - | 担当者ログイン情報（実体） | `dbo.ET0025担当者保存` | 担当者CD, 担当者名, ログインID, PASSWORD | `ET0010担当者`と似た構造だが、実際にログインID/PASSWORDが設定されているのはこちら（全44件中17件にログインIDあり）。**`app.AppUser`の初期投入は、このテーブルのログインID・PASSWORD（平文）をそのまま流用し、パスワードのみbcryptでハッシュ化して登録した**（2026-07-13実施、17件、全件で既存パスワードとの一致を検証済み）。ログインIDが無い担当者（27件）はアプリにログインできない状態のままなので、必要に応じて追加のアカウント発行が必要 |
