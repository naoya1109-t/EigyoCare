...
> - `/api/budget/progress?repCode=&period=` — `period`省略時は`ET0005期`の`現在='1'`の期を使用。達成率は実績データが存在する月のみを対象に計算（未経過月は含まない）。`repCode`省略時は全担当者の予算・実績を合算した「全社計」を返す（2026-07-13追加。フィードバックにより、担当者だけでなく全社の予算進捗も見たいという要望に対応）
