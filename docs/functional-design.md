...
| 4 | 顧客詳細の売掛推移（直近12ヶ月） | `app.ReceivablesByCustomer`（VIEW、本アプリで新規作成） | 得意先CD, 年月, 売上額, 入金額 | `ET0130売掛`を直接クエリせず、ビュー経由でアクセスする方針により新規作成。類似の既存ストアド`ES0120売掛推移`は担当者×会計年度で得意先を横に並べる別用途で、入金額を含まないため流用できなかった |
