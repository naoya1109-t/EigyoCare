...
|---|---|---|---|---|
| - | 担当者 | `dbo.ET0010担当者` | 担当者CD(PK,smallint), 担当者名, ログインID, PASSWORD | ログインID/PASSWORDは平文で保存された既存資産。本アプリの認証には使わず、`app.AppUser`（bcryptハッシュ）を別途使用する |
...
| 9 | 担当者別売上対比（当月） | `dbo.ET0150担当者別売上当月比較` | 担当者CD(PK), 売上金額, 粗利合計 | |
| 4 | 顧客情報（得意先） | `dbo.ET0020得意先` | 得意先CD(PK,int), 得意先名, 郵便番号, 県CD(→`ET0001県`), 住所1/2, TEL, FAX, 営業担当CD, 締日 等（全43列） | |
| - | 県マスタ | `dbo.ET0001県` | 県CD(PK), 県名 | 得意先.県CDのFK先。都道府県はこちらから取得する |
| 5 | 仕入先情報 | `dbo.SV0020仕入先`（VIEW） | 仕入先CD, 仕入先名, 郵便番号, 県CD, 住所1/2, TEL, FAX, 担当者情報 等（全86列） | **仕入先データは別DB（`10.194.5.55` / `Medical` / `dbo.T430仕入先`）にあり**、SaleDB側からはリンクサーバー経由の`SV0020仕入先`ビュー（`SELECT * FROM [10.194.5.55].Medical.dbo.T430仕入先`）でそのまま参照できる。既存の画面ではカナ検索用ストアド`ES0190仕入先リスト`（`@カナ`一文字を受け取りLIKE検索）を使っているが、本アプリでは`SV0020仕入先`ビューを直接クエリすれば全項目を取得できる |
